...
export const load: PageServerLoad = async ({ params, locals: { supabase, safeGetSession } }) => {
  ...
  // Core columns are guaranteed to exist (they're the same set the public list query reads), so
  // existence + the 404 decision never depend on a column that a drifted DB might be missing.
  const CORE = 'id, slug, title, summary_md, type, status, author_id, created_at';
  // Extended columns enrich the page but must NOT be able to 404 it: if the DB is missing any of
  // them (schema drift between migrations and a rebuilt prod DB), the rich select errors and we
  ...
  let idea: Record<string, any> | null = null;
  {
    ...
    if (full.data) {
      ...
      // Rich select failed (likely a missing column). Log it, then fall back to the safe core set.
